...
class ISYAccessoryBaseSetup extends hap_nodejs_1.Accessory {
    ...
    setFanOnState(onState, callback) {
        ...
        if (onState != this.getIsFanOn()) {
            if (onState) {
                ...
                this.setFanRotationSpeed(this.translateFanSpeedToHK(isy_js_1.ISYFanDeviceState.MEDIUM), callback);
            }
            else {
                this.log("FAN: " + this.device.name + " Setting fan speed to off");
                this.setFanRotationSpeed(this.translateFanSpeedToHK(isy_js_1.ISYFanDeviceState.OFF), callback);
            }
        }
        else {
